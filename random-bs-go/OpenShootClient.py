...
def server():
    ...
    while True:
        ...
        data = data.split(";")
        pixel = (int((float(data[0])*-1)/3*271),
                 int((float(data[1])*-1)/3*271))

        shot.append((pixel[0], pixel[1], time.time(), data[2]))
...
def ui():
    ...
    button = tkinter.Button(tk, text="Clear", command=clear,
                            bg="white", fg="black", height=5, width=10)
    ...
    while True:
        ...
        if len(shot) > 1:
            for i, point in enumerate(shot):
                ...
                x2, y2, _ = shot[i-1]
                canvas.create_line(
                    int(x)+400, int(y)+400, int(x2)+400, int(y2)+400, fill="#F00", width=1)
